...
{if $list|@count}
  ...
  {if $canAddTopic }
  <DIV ALIGN="RIGHT">
  <input class="button button-add" onclick="self.location='{copixurl dest="|getTopicForm" forum=$forum->id}'" type="button" value="{i18n key="forum.btn.addTopic"}" />
  </DIV>
  {/if}
...
{else}
  ...
  <p>{i18n key="forum.list.noTopic"}</p>

  {if $canAddTopic}<p><input class="button button-add" onclick="self.location='{copixurl dest="|getTopicForm" forum=$forum->id}'" type="button" value="{i18n key="forum.btn.addTopic"}" /></p>{/if}
{/if}
