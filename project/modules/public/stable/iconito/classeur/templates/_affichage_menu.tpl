<div id="submenu">
  <div class="menuitems">
    <ul>
      ...
      {/if}
      <li><a class="viewList{if $ppo->current eq "liste"} current{/if}" href="{copixurl dest="classeur||voirContenu" classeurId=$ppo->classeurId dossierId=$ppo->dossierId vue=liste}"><span class="valign"></span><span>{i18n key="classeur.message.listView"}</span></a></li>
      <li><a class="viewVign{if $ppo->current eq "vignette"} current{/if}" href="{copixurl dest="classeur||voirContenu" classeurId=$ppo->classeurId dossierId=$ppo->dossierId vue=vignette}"><span class="valign"></span><span>{i18n key="classeur.message.thumbnailView"}</span></a></li>
      {if $ppo->niveauUtilisateur >= PROFILE_CCV_MODERATE}
        <li><a class="image{if $ppo->current eq "editerAlbumPublic"} current{/if}" href="{copixurl dest="classeur||editerAlbumPublic" classeurId=$ppo->classeurId dossierId=$ppo->dossierId}"><span class="valign"></span><span>{i18n key="classeur.message.createPublicAlbum"}</span></a></li>
      {/if}
    </ul>
  </div>
</div>