  ...
  <tbody>
    ...
    {if $fichier->estUnFavori()}
      <tr class="{$fichier->type} {if $index%2 eq 0}odd{else}even{/if}">
        ...
        <td><a href="{$fichier->getLienFavori()}" title="{i18n key="classeur.message.openFile" titre=$fichier->titre noEscape=1}">{$fichier->titre|escape}</a></td>
        <td>Favori</td>
    ...
    {else}
      <tr class="{$fichier->type} {if $index%2 eq 0}odd{else}even{/if}">
        <td><input type="checkbox" class="check" name="fichiers[]" value="{$fichier->id}" /></td>
        <td><a href="{copixurl dest="classeur||telechargerFichier" classeurId=$ppo->classeurId fichierId=$fichier->id}" title="{i18n key="classeur.message.openFile" titre=$fichier->titre noEscape=1}">{$fichier->titre|escape}</a></td>
