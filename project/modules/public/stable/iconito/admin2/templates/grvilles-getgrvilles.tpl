...
<tr height="100">
	<td width="30%" bgcolor="white" valign="top">
		<div class="button_like"><a href="{copixurl dest="admin2|grvilles|" groupe="0"}" class="button_like">Nouveau regroupement</a></div>
		<div>
			<h2>Regroupements existants</h2>
			{if $grvilles_list|@count gt 0}
				<ul>
				{foreach from=$grvilles_list item=grvilles_item}
					<li><a href="{copixurl dest="admin2|grvilles|" groupe=$grvilles_item->id}">{$grvilles_item->nom}</a></li>
				{/foreach}
				</ul>
			{/if}
		</div>
	...
	<td width="70%" bgcolor="white" valign="top">
		...
		<form method="POST">
			...
			<p id="delete_confirm" style="display: none;">
				<a href="{copixurl dest="admin2|grvilles|" delete=$grvilles_infos->id}" class="confirm">Confirmez la suppression</a>
