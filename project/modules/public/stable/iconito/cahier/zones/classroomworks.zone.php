 ...
 *
 */
class ZoneClassroomWorks extends CopixZone {
    ...
    if (Kernel::isParent()) {
	  ...
	  if (Kernel::isEleve()) {
	    ...
	    $ppo->works = $workDAO->findWorksForStudent($ppo->nid, DAOCahier_travail::TYPE_CLASSROOM, $time);
	  }
	  elseif (Kernel::isParent()) {
	    
	    $ppo->works = $workDAO->findWorksForStudent($studentId, DAOCahier_travail::TYPE_CLASSROOM, $time);
	  }
	  elseif (Kernel::isEnseignant()) {
	    
	    $ppo->works = $workDAO->findWorksForClassroom($ppo->nid, DAOCahier_travail::TYPE_CLASSROOM, $time);
	  }

	  $toReturn = $this->_usePPO ($ppo, '_classroom_works.tpl');
  }
}