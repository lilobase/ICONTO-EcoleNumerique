...
{foreach from=$list item=ecole}
	{if $ecole.id>0}
		...
		{if $ajax}<a href="{copixurl dest="fichesecoles||fiche" id=$ecole.id}" onClick="return ajaxFicheEcole({$ecole.id});">{$ecole.nom}</a>{else}<a href="{copixurl dest="fichesecoles||fiche" id=$ecole.id}">{$ecole.nom}</a>{/if}{if $dispType && $ecole.type} ({$ecole.type}){/if}<br/>
