...
<table border="0" CLASS="liste" ALIGN="CENTER" CELLSPACING="2" CELLPADDING="2" STYLE="width:84%;">
	<tr>
		...
		{foreach from=$list item=item}
			...
			<TD>{if ($item->last_visite eq null or $item->last_visite<$item->last_msg_date)}<a title="{i18n key="carnet.list.goNewTitle"}" href="{copixurl dest="|getTopic" id=$item->id eleve=$eleve go=new}"><IMG src="{copixresource path="img/icon_new.gif"}" WIDTH="17" HEIGHT="17" BORDER="0" /></A>{/if}</TD>
			<td>
