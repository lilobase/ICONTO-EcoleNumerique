...
{foreach from=$arFlux item=news}
  ...
  {/if}
	...
	<img src="img/blog/enclosure.gif" border="0" width="16" height="16" style="padding:0px;" /><a href="{$encl.url}" type="{$encl.type}">{$encl.url|substrpos}</a> ({$encl.length|human_file_size})
	{/foreach}
	</div>
	{/if}
