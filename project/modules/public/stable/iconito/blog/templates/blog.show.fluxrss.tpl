		   ...
		   <tr>
			 ...
			 {assign var=cptRss value=0}
			   ...
			   <tr class="list_line{$cptRss%2}" {cycle values=',class="alternate"' name="resultats"}>
                   ...
                   <td class="action">{if $cptRss>0}<a class="button button-up" href="{copixurl dest=blog|admin|upRss id_bfrs=$rss->id_bfrs id_blog=$id_blog kind=$kind}">{i18n key=copix:common.buttons.moveup}</a>{/if}</td>
		           <td class="action">{if $cptRss<$tabRss|@count-1}<a class="button button-down" href="{copixurl dest=blog|admin|downRss id_bfrs=$rss->id_bfrs id_blog=$id_blog kind=$kind}">{i18n key=copix:common.buttons.movedown}</a>{/if}</td>
			   </tr>
