...
class ical_parser {
	...
	function chooseOffset($time) {
		global $tz_array;
		$timezone = CopixConfig::get('agenda|timeZone');
		switch ($timezone) {
			case '':
				$offset = 'none';
				break;
	...
	function parse($cal_filelist) {
		...
		$calnumber = 1;
		foreach ((array)$cal_filelist as $cal_key=>$filename) {
			...
			if ($parse_file) {	
				...
				if ($ifile == FALSE){//fichier introuvable
					return $ifile;
				...
				switch ($line) {
					...
					case 'END:VEVENT':
						...
						if (isset($start_time) && $start_time != '') {
							...
							/*$drawKey = $this->drawEventTimes($start_time, $end_time);
