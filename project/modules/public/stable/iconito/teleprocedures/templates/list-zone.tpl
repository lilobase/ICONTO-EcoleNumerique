 ...
 {foreach from=$liste item=r}
    ...
    <td class="discret" align="center">{$r->dateinter|datei18n:"date_short"}</td>
	  ...
	  {else} {$r->datederniere|datei18n:"date_short"} {/if}</td>
		...
		<td>{if $r->last_visite eq null or $r->last_visite<$r->datederniere}<img src="{copixresource path="img/icon_new.gif"}" width="17" height="17" border="0" align="right" />{/if}<a title="Fiche" href="{copixurl dest="|fiche" id=$r->idinter}">{$r->objet|escape}</a></td>	 
...
<img src="{copixresource path="img/icon_new.gif"} width="14" height="14" border="0" /> du neuf &agrave; lire
