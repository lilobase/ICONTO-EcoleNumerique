...
class DAORessource_Tags {
	...
	function getSimilarTags ($tag, $annu=0) {
		...
		$ressources_list = $dbw->fetchAll ($critere);
		...
		$tags_list = $dbw->fetchAll ($critere);
		$tags_keys = array();
...
/*
		$critere = 'SELECT COUNT( DISTINCT t_count.res ) AS nb, t_count.tag';
		$critere.= ' FROM module_ressource_tags AS t_tag';
		$critere.= ' LEFT JOIN module_ressource_tags AS t_res';
		$critere.= '  ON t_tag.res=t_res.res';
		$critere.= ' LEFT JOIN module_ressource_tags AS t_count';
		$critere.= '  ON t_res.tag=t_count.tag';
		$critere.= ' WHERE t_tag.tag=\''.$tag.'\' and t_res.tag!=\''.$tag.'\'';
		
		if( $annu > 0 ) $critere.= ' AND t_tag.annu='.$annu;
		if( $annu > 0 ) $critere.= ' AND t_res.annu='.$annu;
		if( $annu > 0 ) $critere.= ' AND t_count.annu='.$annu;
		
		$critere.= ' GROUP BY tag';
...
*/
	...
	function getTagsForRessource ($res, $annu=0) {
		$res = addslashes( $res );
		$dbw = & CopixDbFactory::getDbWidget ();
...
class TagResult {
	...
	function __construct( $local_tag, $local_nb ) {
		$tag = $local_tag;
		$nb  = $local_nb;
	}
}
