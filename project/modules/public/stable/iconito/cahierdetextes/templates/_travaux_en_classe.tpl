<h3>{i18n key="cahierdetextes.message.classroomWork"} {if $ppo->typeUtilisateur == 'USER_ENS'}<a class="button button-add" href="{copixurl dest="cahierdetextes||editerTravail" nid=$ppo->nid jour=$ppo->jour mois=$ppo->mois annee=$ppo->annee vue=$ppo->vue}">{i18n key="cahierdetextes.message.addClassroomWork"}</a>{/if}</h3>

<div class="classroom-works-list">
  {if $ppo->travaux neq null}
    {foreach from=$ppo->travaux item=travail}
      <div class="work">
        <h4>{$travail->nom} {if $ppo->typeUtilisateur == 'USER_ENS'}
          <span class="actions">
