...
<div class="menuitems">
    <ul>
      ...
      <li class="newGroupItems"><a class="{if $ppo->current == "voirTravaux"} current{/if}" href="{copixurl dest="cahierdetextes||voirTravaux" cahierId=$ppo->cahierId jour=$ppo->jour mois=$ppo->mois annee=$ppo->annee}"><span class="valign"></span><span>{i18n key="cahierdetextes.message.day"}</span></a></li>
      <li><a class="{if $ppo->current == "voirListeTravaux"} current{/if}" href="{copixurl dest="cahierdetextes||voirListeTravaux" cahierId=$ppo->cahierId jour=$ppo->jour mois=$ppo->mois annee=$ppo->annee}"><span class="valign"></span><span>{i18n key="cahierdetextes.message.list"}</span></a></li>
      <li><a class="{if $ppo->current == "voirTravauxParDomaine"} current{/if}" href="{copixurl dest="cahierdetextes||voirTravauxParDomaine" cahierId=$ppo->cahierId jour=$ppo->jour mois=$ppo->mois annee=$ppo->annee}"><span class="valign"></span><span>{i18n key="cahierdetextes.message.domains"}</span></a></li>
