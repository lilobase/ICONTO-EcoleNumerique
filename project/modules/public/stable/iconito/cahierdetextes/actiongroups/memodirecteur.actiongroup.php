...
class ActionGroupMemoDirecteur extends CopixActionGroup
{
    ...
    public function beforeAction($actionName)
    {
        ...
        var_dump(CopixSession::get('myNode'));
    ...
    public function processVoir()
    {
        ...
        $ppo->isDirecteurOfClasse = Kernel::isDirecteurOfClasse($ppo->nodeId);
        ...
        if (CopixRequest::isMethod('post')) {
            ...
            $memo2eleve    = $memo2eleveDAO->get(_request('memoId', null), $ppo->eleve);
            if (is_null($memo2eleve) || !Kernel::isParentOfEleve($ppo->eleve)) {
                return CopixActionGroup::process('generictools|Messages::getError', array(
                    'message' => CopixI18N::get('kernel|kernel.error.errorOccurred'),
                    'back'    => CopixUrl::get('')
                ));
            ...
            return _arRedirect(CopixUrl::get('cahierdetextes|memodirecteur|voir', array('cahierId' => $ppo->cahierId)));
        }
