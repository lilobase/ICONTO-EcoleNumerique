...
class DAOCahierDeTextesMemo2eleve {
   ...
   */
	public function findSuiviElevesParMemo ($idMemo) {
      ...
      . ' FROM kernel_bu_eleve E, kernel_bu_eleve_affectation A, kernel_link_bu2user LI, dbuser U, kernel_bu_classe_niveau CN, kernel_bu_ecole_classe CL, module_cahierdetextes_memo2eleve AS M, kernel_bu_ecole_classe_niveau ECN'
		  . ' WHERE E.idEleve = A.eleve'
		  . ' AND A.eleve = M.kernel_bu_eleve_idEleve'
		  . ' AND A.classe = CL.id'
		  . ' AND CL.id = ECN.classe'
		  . ' AND A.niveau = CN.id_n'
		  . ' AND A.current = 1'
		  . ' AND ECN.niveau = CN.id_n'
