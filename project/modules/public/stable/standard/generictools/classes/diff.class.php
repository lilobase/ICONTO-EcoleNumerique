<?php
/**
* diff.class.php is a diff system comparing two strings
* and can apply differences as patch
* @package		standard
 * @subpackage	generictools
* @author Geoffrey T. Dairiki, Patrice Ferlet
* @copyright Copyright (C) 2000 Geoffrey T. Dairiki <dairiki@dairiki.org>, Copyright &copy; 2006, CopixTeam
*/

// diff.php taken from phpwiki
//
...
// Only to have a better syntax: add Diff class and some functions
// are overloaded
// by Patrice Ferlet
 ...
 * @subpackage	generictools
 */
class Diff extends Wikidiff
{
    public function Diff($from_lines = false, $to_lines = false)
    {
        if($from_lines!==false and !is_array($from_lines) and $to_lines){
            $from_lines = explode("\n",$from_lines);
        }
        if($to_lines!== false and !is_array($to_lines)){
            $to_lines = explode("\n",$to_lines);
        }
        parent::WikiDiff($from_lines, $to_lines);
    }
}
...
class _WikiDiffEngine
{
    ...
    public function _WikiDiffEngine($from_lines, $to_lines)
    {
...
class WikiDiff
{
    ...
    public function WikiDiff($from_lines = false, $to_lines = false)
    {
        ...
        for (reset($this->edits); $edit = current($this->edits); next($this->edits)) {
            if (is_array($edit)) { // Was an add, turn it into a delete.
                ...
                if ($edit > 0) {
                    ...
                    if ($edit < 0) { // Was a delete, turn it into an add.
                        $ndelete = - $edit;
                        $edit = array ();
                        while ($ndelete-- > 0)
                            $edit[] = "" . $from_lines[$x++];
    ...
    public function compose($that)
    {
        ...
        while ($left || $right) {
            if (!is_array($left) && $left < 0) { // Left op is a delete.
                ...
                if (is_array($right)) { // Right op is an add.
                    $newop = $right;
                    ...
                    if (!$left || !$right)
                        ...
                        if (!is_array($left) && $left > 0) { // Left op is a copy.
                            if ($left <= abs($right)) {
            ...
            if (is_array($op) && is_array($newop)) {
                ...
                if (($op > 0 && $newop > 0) || ($op < 0 && $newop < 0)) { // $op and $newop are both either deletes or copies.
                    $op += $newop;
                } else {
                    $comp->edits[] = $op;
                    $op = $newop;
    ...
    */
    ...
    public function apply($from_lines)
    {
        ...
        if ($x != $xlim)
            die( sprintf("WikiDiff::apply: line count mismatch: %s != %s", $x, $xlim ));
        return $output;
    }
...
class WikiDiffFormatter
{
    ...
    public function WikiDiffFormatter($reverse = false)
    {
            ...
            if (!empty ($hunk)) {
                if (!$next || $ncopy > 2 * $this->context_lines) {
                    ...
                    if ($this->do_reverse_diff)
                        list ($xbeg, $xlen, $ybeg, $ylen) = array (
                            $ybeg,
                            $ylen,
                            $xbeg,
                            $xlen
    ...
    {
        $html = '<tr><td><table style="background-color: white"' . ' cellspacing="0" border="0" cellpadding="4">' . '<tr bgcolor="#cccccc"><td><tt>' . $this->_diff_header($xbeg, $xlen, $ybeg, $ylen) . "</tt></td></tr>\n<tr><td>\n" . '<table cellspacing="0" border="0" cellpadding="2">';
...
{
    public function WikiUnifiedDiffFormatter($reverse = false, $context_lines = 3)
    {
        $this->do_reverse_diff = $reverse;
