...
$dossierInstall=realpath(dirname(realpath($_SERVER['SCRIPT_FILENAME'])).'/../');
...
if (isset($_POST['do'])) {
	echo "C'est parti !!";
	exec("svn --username=$loginSVN --password=$passwordSVN up $dossierInstall", $svnUp, $error);	
	rsort($svnUp);
	echo "<pre>";
 ...
 **/
exec("svn info $dossierInstall", $workingCopy, $error);
...
exec("svn --username=$loginSVN --password=$passwordSVN info $WC_URL", $REPO, $error);
...
if ($REPO_REV > $WC_REV) {
        ...
        exec("svn --username=$loginSVN --password=$passwordSVN status -uq $dossierInstall", $svnStatus, $error);
        array_pop($svnStatus);
        sort($svnStatus);
        $db=0;
...
if (isset($_POST['detailSVN'])) {
	exec("svn --username=$loginSVN --password=$passwordSVN diff $dossierInstall/".$_POST['detailSVN'], $svnDiff, $error);	
 ...
 **/
function accessDB() {
	$installDir = realpath(dirname(realpath($_SERVER['SCRIPT_FILENAME'])).'/../'); 
